...
cd 'BuildOutput\Artifacts\src\ARM\Infrastructure\Environments\TTMInsights\PROD'
...
$ResourceGroupName = 'amn-wus2-ttm-rg-p01'
...
#$TemplateFile = "$TemplatePath\vnet.template.json"
#$TemplateParametersFile = "vnet.parameters.json"
#$DeploymentName = ((Get-ChildItem $TemplateFile).BaseName + '-' + ((Get-Date).ToUniversalTime()).ToString('MMdd-HHmm'))
#New-AzResourceGroupDeployment -Name $DeploymentName `
#    -ResourceGroupName $ResourceGroupName `
#    -TemplateFile $TemplateFile `
